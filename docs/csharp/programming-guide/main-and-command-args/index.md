...
- `Main`은 클래스 또는 구조체 내부에 선언됩니다 `Main`은 [정적](../../language-reference/keywords/static.md)이어야 하며 [공용](../../language-reference/keywords/public.md)일 필요가 없습니다. (앞의 예제에서는 기본 액세스 한정자인 [private](../../language-reference/keywords/private.md)로 지정됩니다.) 바깥쪽 클래스 또는 구조체는 정적일 필요가 없습니다.
- `Main`은 `void` 또는 `int`를 반환 형식을 가질 수 있으며 C# 7.1 부터는 `Task` 또는 `Task<int>` 반환 형식을 가질 수 있습니다.
